...
def check_forbidden_content(text: str) -> RuntimeAuditForbiddenContentCheck:
    ...
    return RuntimeAuditForbiddenContentCheck(
        scanned=True,
        clean=len(found) == 0,
        forbidden_patterns_found=found,
    )
        ...
        for lm in _PROM_LABEL_RE.finditer(labels_str):
            ...
            if not val_check.clean:
                forbidden_labels.append(
                    f"{key}_value_forbidden:{val_check.forbidden_patterns_found[0][:32]}"
                )
        try:
...
async def probe_docker(
    ...
    try:
        ...
        return RuntimeAuditDockerProbe(
            status=RuntimeAuditProbeStatus.ERROR,
            docker_available=False,
            message=f"Docker error: {type(exc).__name__}",
        )
...
async def probe_log_tail(
    ...
    try:
        ...
        error_count = sum(
            1
            for line in lines
            if '"level": "error"' in line.lower() or "error" in line.lower()[:20]
        )
        warning_count = sum(
            1
            for line in lines
            if '"level": "warning"' in line.lower() or "warning" in line.lower()[:20]
        )
...
async def summarize_ledger(
    ...
    try:
        ...
        error_count = sum(
            1 for e in events if e.severity == OperationalEventSeverity.ERROR
        )
        warning_count = sum(
            1 for e in events if e.severity == OperationalEventSeverity.WARNING
        )
        ws_reconnect = sum(
            1 for e in events if e.event_type == OperationalEventType.WS_RECONNECT
        )
        budget_block = sum(
            1
            for e in events
        ...
        readiness = sum(
            1
            for e in events
            if e.event_type == OperationalEventType.READY_STATE_CHANGED
        )
        alert = sum(
        ...
        return RuntimeAuditLedgerSummary(
            ...
            error_count=error_count,
            warning_count=warning_count,
            ws_reconnect_count=ws_reconnect,
            budget_block_count=budget_block,
            provider_failure_count=provider_fail,
            market_quarantine_count=market_q,
            readiness_change_count=readiness,
            alert_count=alert,
        ...
        return RuntimeAuditLedgerSummary(
            available=False,
            message=f"Ledger read failed: {type(exc).__name__}",
        )
...
async def summarize_decision_repository(
    ...
    try:
        ...
        for d in decisions:
            action = getattr(
                d.recommended_action, "value", str(d.recommended_action)
            ).upper()
            if action == "BUY":
                buy += 1
            elif action == "SELL":
        ...
        return RuntimeAuditDecisionSummary(
            available=False,
            message=f"Decision read failed: {type(exc).__name__}",
        )
...
async def summarize_market_repository(
    ...
    try:
        ...
        return RuntimeAuditMarketSummary(
            available=False,
            message=f"Market read failed: {type(exc).__name__}",
        )
...
async def summarize_position_repository(
    ...
    try:
        ...
        return RuntimeAuditPositionSummary(
            ...
            available=False,
            message=f"Position read failed: {type(exc).__name__}",
        )
...
async def summarize_execution_repository(
    ...
    try:
        ...
        return RuntimeAuditExecutionSummary(
            available=False,
            message=f"Execution read failed: {type(exc).__name__}",
        )
...
def write_audit_artifacts(
    ...
    if not json_check.clean or not md_check.clean:
        combined = (
            json_check.forbidden_patterns_found + md_check.forbidden_patterns_found
        )
        return RuntimeAuditArtifactWriteResult(
            success=False,
            ...
            forbidden_content_check=RuntimeAuditForbiddenContentCheck(
                scanned=True,
                clean=False,
                forbidden_patterns_found=combined,
            ),
        )
    ...
    try:
        ...
        latest_failure = f"{latest_failure}; {detail}" if latest_failure else detail
...
async def send_telegram_alert(
    ...
    if not forbidden.clean:
        return RuntimeAuditTelegramResult(
            sent=False,
            reason="forbidden_content_in_payload",
        )
...
async def run_audit(
    *,
        ...
        if not health.reachable:
            findings.append(
                RuntimeAuditFinding(
                    ...
                    source="health",
                    failure_reason=RuntimeAuditFailureReason.HEALTH_PROBE_ERROR,
                )
            )
        else:
            findings.append(
    ...
    probe_error_findings = [
        f
        for f in findings
        if f.finding_type == RuntimeAuditFindingType.PROBE_ERROR
    ...
    if safety_gate_findings:
        ...
        f.severity in (RuntimeAuditSeverity.ERROR, RuntimeAuditSeverity.WARNING)
        for f in findings
    ):
    ...
    cooldown_block_rate = _compute_cooldown_block_rate(ledger_summary, decision_summary)
    ...
    telegram_result = await send_telegram_alert(
        notifier, report, enabled=telegram_enabled
    )
