---
name: "source-command-dry-run-review"
...
# source-command-dry-run-review

Use this skill when the user asks to run the migrated source command `dry-run-review`.
...
13. **Append a session summary to today's daily note** at `~/documents/integration_task/03_Daily/{today}.md`, using the AGENTS.md Session End format:
    ```
...
- NEVER use `print()` in any generated code sketch — use `structlog` (per AGENTS.md).
