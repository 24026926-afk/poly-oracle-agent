...
def _run(d: Path, hours: int = 72) -> tuple[int, dict, str]:
    r = subprocess.run(
        [PY, str(SCRIPT), "--hours", str(hours), "--artifact-dir", str(d)],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def _art(
    *,
    ...
    return {
        ...
        "decision_summary": {
            "total_decisions": sum(dec.values()),
            **{f"{k}_count": v for k, v in dec.items()},
        },
        "position_summary": {
            "open_count": 1,
            "settled_count": 0,
            "total_open_exposure_usdc": exposure,
        },
        "database_probe": {
            "status": "SUCCESS",
            "file_exists": True,
            "file_size_bytes": db_bytes,
        },
    }
...
class TestAggregatorHappyPath:
    ...
    def test_total_errors_aggregated_across_artifacts(self, tmp_path):
        ...
        _, d, _ = _run(tmp_path)
        assert d["total_errors"] == 8


class TestZeroArtifactDetection:
    def test_exits_one_when_no_artifacts(self, tmp_path):
        c, d, _ = _run(tmp_path)
        assert c == 1 and d.get("error") == "no_artifacts_in_window"

    def test_error_json_has_no_artifacts_in_window(self, tmp_path):
        c, d, _ = _run(tmp_path)
        assert c == 1 and "no_artifacts_in_window" in json.dumps(d)

    def test_exits_one_when_directory_missing(self, tmp_path):
        c, d, _ = _run(tmp_path / "nonexistent")
        assert c == 1 and "error" in d

    def test_exits_one_when_artifacts_outside_window(self, tmp_path):
        _write(
            tmp_path,
            "runtime-audit-20200101-000000.json",
            _art(ts="2020-01-01T00:00:00+00:00"),
        )
        c, d, _ = _run(tmp_path, hours=1)
        assert c == 1 and d.get("error") == "no_artifacts_in_window"
...
class TestMalformedArtifactHandling:
    ...
    def test_valid_artifacts_still_processed(self, tmp_path):
        ...
        c, d, _ = _run(tmp_path)
        assert c == 0 and d["total_errors"] == 7

    def test_all_malformed_treated_as_zero_artifacts(self, tmp_path):
        for i in range(3):
            (tmp_path / _fn(i + 1)).write_text("garbage", encoding="utf-8")
        c, d, _ = _run(tmp_path)
        assert c == 1 and d.get("error") == "no_artifacts_in_window"
...
class TestSecretScrubbing:
    def test_wallet_addresses_redacted(self, tmp_path):
        ...
        _write(tmp_path, _fn(), a)
        _, _, raw = _run(tmp_path)
        assert "0x" + "a" * 40 not in raw
...
class TestDecisionDistribution:
    def test_decision_distribution_all_zeros(self, tmp_path):
        ...
        _, d, _ = _run(tmp_path)
        assert all(
            d["decision_distribution"][k] == 0 for k in ("buy", "sell", "hold", "skip")
        )
    ...
    def test_decision_distribution_buy_count(self, tmp_path):
        _write(
            tmp_path,
            _fn(),
            _art(decisions={"buy": 5, "sell": 0, "hold": 0, "skip": 0}, ts=_ts()),
        )
        _, d, _ = _run(tmp_path)
        assert d["decision_distribution"]["buy"] == 5

    def test_decision_distribution_sell_count(self, tmp_path):
        _write(
            tmp_path,
            _fn(),
            _art(decisions={"buy": 0, "sell": 3, "hold": 0, "skip": 0}, ts=_ts()),
        )
        _, d, _ = _run(tmp_path)
        assert d["decision_distribution"]["sell"] == 3

    def test_decision_distribution_hold_count(self, tmp_path):
        _write(
            tmp_path,
            _fn(),
            _art(decisions={"buy": 0, "sell": 0, "hold": 7, "skip": 0}, ts=_ts()),
        )
        _, d, _ = _run(tmp_path)
        assert d["decision_distribution"]["hold"] == 7

    def test_decision_distribution_skip_count(self, tmp_path):
        _write(
            tmp_path,
            _fn(),
            _art(decisions={"buy": 0, "sell": 0, "hold": 0, "skip": 9}, ts=_ts()),
        )
        _, d, _ = _run(tmp_path)
        assert d["decision_distribution"]["skip"] == 9

    def test_decision_distribution_aggregated_across_artifacts(self, tmp_path):
        _write(
            tmp_path,
            _fn(1),
            _art(decisions={"buy": 2, "sell": 1, "hold": 0, "skip": 0}, ts=_ts(1)),
        )
        _write(
            tmp_path,
            _fn(2),
            _art(decisions={"buy": 3, "sell": 2, "hold": 1, "skip": 4}, ts=_ts(2)),
        )
        _, d, _ = _run(tmp_path)
...
class TestDBGrowthDelta:
    ...
    def test_db_growth_delta_negative_shrinkage(self, tmp_path):
        ...
        _, d, _ = _run(tmp_path)
        assert Decimal(d["db_growth_bytes_delta"]) == Decimal("-500")
...
class TestStreamingProcessing:
    def test_large_artifact_set_does_not_spike_memory(self, tmp_path):
        for i in range(100):
            _write(tmp_path, _fn(i + 1), _art(ts=_ts(i + 1)))
        c, d, _ = _run(tmp_path, hours=120)
        assert c == 0 and d["scanned_files"] == 100

    def test_accumulator_pattern_used(self, tmp_path):
        for i in range(10):
            _write(tmp_path, _fn(i + 1), _art(errors=i, ts=_ts(i + 1)))
        _, d, _ = _run(tmp_path)
        assert d["total_errors"] == sum(range(10))
...
class TestEdgeCases:
    def test_single_artifact_in_window(self, tmp_path):
        ...
        c, d, _ = _run(tmp_path)
        assert c == 0 and Decimal(d["db_growth_bytes_delta"]) == Decimal("0")
    ...
    def test_timezone_naive_treated_as_utc(self, tmp_path):
        a = _art(ts=_ts())
        a["generated_at_utc"] = a["generated_at_utc"].replace("+00:00", "")
        _write(tmp_path, _fn(), a)
        c, d, _ = _run(tmp_path)
        assert c == 0
    ...
    def test_dry_run_true_documented_not_flagged(self, tmp_path):
        ...
        _, d, _ = _run(tmp_path)
        assert d["dry_run_posture"] is True and d.get("dry_run_inconsistent") is False
    ...
    def test_dry_run_inconsistency_flagged(self, tmp_path):
        ...
        _, d, _ = _run(tmp_path)
        assert d.get("dry_run_inconsistent") is True

    def test_max_exposure_absent_reports_unavailable(self, tmp_path):
        a = _art(ts=_ts())
        del a["position_summary"]
        _write(tmp_path, _fn(), a)
        _, d, _ = _run(tmp_path)
        assert d["max_exposure_usdc"] == "unavailable"
